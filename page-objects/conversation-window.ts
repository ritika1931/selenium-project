...
const chatBoxAutomatedMesgsElement =
  "//*[@class='message__body ng-star-inserted']";
const userAudioMesgsElement =
  "//*[@class='message__body message__body_audio ng-star-inserted']";
const chatBotMesgText = "//*[@class='message__body ng-star-inserted']/p";
const counterIconOnMic =
  "//*[@class='mat-mini-fab mat-button-base mat-primary ng-star-inserted']/span";
const userInputTextMesg =
  "//*[@class ='message message--right ng-star-inserted']/div/div/div/p";
const chatBotResponseTextMesg =
  "//*[@class='message ng-star-inserted']/div/div/div/p";
const firstButtonForSelection =
  "//*[@data-test='message-suggested-btn'][1]/span";
...
async function getChatBotIframe(driver: WebDriver) {
  const IframeElement = await DriverUtils.findElementByXPath(
    driver,
...
async function clickChatBotCloseIcon(driver: WebDriver) {
  const closeIconElement = await DriverUtils.findElementById(
    driver,
...
async function getUserLastMesgElement(driver: WebDriver) {
  const microphoneInputElements: WebElement[] =
    await DriverUtils.findElementsByXPath(driver, userAudioMesgsElement);
...
async function getAutomatedLastMesgElement(driver: WebDriver) {
  const microphoneInputElements: WebElement[] =
    await DriverUtils.findElementsByXPath(driver, chatBoxAutomatedMesgsElement);
  return microphoneInputElements[microphoneInputElements.length - 1];
}

async function getUserTextMesg(driver: WebDriver) {
  const microphoneInputElements: WebElement[] =
    await DriverUtils.findElementsByXPath(driver, userInputTextMesg);
...
async function getChatBotTextMesg(driver: WebDriver) {
  const microphoneInputElements: WebElement[] =
    await DriverUtils.findElementsByXPath(driver, chatBotResponseTextMesg);
  return microphoneInputElements[microphoneInputElements.length - 1].getText();
}

async function getChatBotTextMesgDate(driver: WebDriver) {
  const microphoneInputElements: WebElement[] =
    await DriverUtils.findElementsByXPath(driver, mesgDate);
  return microphoneInputElements[microphoneInputElements.length - 1].getText();
}
