...
describe('Test conversation flow on chat bot window', async () => {
    ...
    it('1: Verify chat bot message and user message for first conversation', async () => {
        ...
        const actualUserMesg =  await ConversationPageObjects.getUserTextMesg(driver);
        const actualAutomatedMesg =  await ConversationPageObjects.getChatBotTextMesg(driver);
        MochaUtils.verifyEquals(actualUserMesg, CHAT_WINDOW_TEST_DATA.conversation1.userQuery, "Input text box icon is not getting disabled")
        MochaUtils.verifyEquals(actualAutomatedMesg, CHAT_WINDOW_TEST_DATA.conversation1.chatBotResponse, "Input text box icon is not getting disabled")
    })

    it('2: Verify the provided name in further conversations', async () => {
        await DriverUtils.enterValue(driver, ConversationPageObjects.chatBotInputBox, CHAT_WINDOW_TEST_DATA.conversation2.userQuery);
        const sendButtonElement = await DriverUtils.findElementByXPath(driver, ConversationPageObjects.sendTextButton);
        await sendButtonElement.click();
        await driver.sleep(5000);
        const actualUserMesg =  await ConversationPageObjects.getUserTextMesg(driver);
        ...
        MochaUtils.verifyEquals(actualUserMesg, CHAT_WINDOW_TEST_DATA.conversation2.userQuery, "Input text box icon is not getting disabled")
        MochaUtils.verifyEquals(actualAutomatedMesg, CHAT_WINDOW_TEST_DATA.conversation2.chatBotResponse, "Input text box icon is not getting disabled")
    })

    it('3: Verify menu button is displayed and enabled on conversation window', async () => {
        const element = await DriverUtils.findElementByXPath(driver, ConversationPageObjects.menuButtonIcon);
