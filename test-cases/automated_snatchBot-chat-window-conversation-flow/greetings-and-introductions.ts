  ...
  it('1: Verify welcome messages are showing up correctly when chat box window opens up', async () => {
    try {
      ...
      MochaUtils.verifyEquals(
        allMessages[0],
        CHAT_WINDOW_TEST_DATA.welcomeMesg1,
        'Welcome message 1 is not showing up correctly'
      );
      MochaUtils.verifyEquals(
        allMessages[1],
        CHAT_WINDOW_TEST_DATA.welcomeMesg2,
        'Welcome message 2 is not showing up correctly'
      );
      MochaUtils.verifyEquals(
        allMessages[2],
        CHAT_WINDOW_TEST_DATA.welcomeMesg3,
        'Welcome message 3 is not showing up correctly'
      );
      MochaUtils.verifyEquals(
        messageElements.length,
        3,
        'All the three messages are not showing up'
      );
    } catch (error) {
      console.error(error);
    }
