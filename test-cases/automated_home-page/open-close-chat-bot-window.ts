...
describe('Test open and close of chat bot window is working successfully', async () => {
    ...
    it('1: Verify close icon is showing when chat bot window opens up', async () => {
        ...
        const closeIconElement = await DriverUtils.findElementByXPath(driver, ConversationPageObjects.chatbotCloseWindowIcon);
        const isIConDisplayed = await AssertUtils.elementIsDisplayed(closeIconElement);
        MochaUtils.verifyIsTrue(isIConDisplayed , "Close window icon is not showing up on chat bot window");
    })
    ...
    it('2: Verify chat bot window is closing successfully', async () => {
        ...
        MochaUtils.verifyEquals(getDisplayAttribute , "none", "Close window icon is not showing up on chat bot window");
    })
