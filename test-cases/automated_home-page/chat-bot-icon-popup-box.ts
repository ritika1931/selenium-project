...
describe('Test if snatch bot pop up box and icon are showing correctly', async () => {
  ...
  beforeEach('Wait for snatchBotPopUpBox to load', async () => {
    await WaitUtils.waitUntilElementIsVisible(
      driver,
      await DriverUtils.findElementByXPath(
        driver,
        HOMEPAGE_LOCATOR.snatchBotPopUpBox
      ),
      10000
    );
  });
  ...
  it('1. Verify snatch bot icon showing pop up circle', async () => {
    const eleExists = await AssertUtils.isElementExists(
      driver,
      HOMEPAGE_LOCATOR.snatchBotPopUp_circle
    );
    MochaUtils.verifyIsTrue(
      eleExists,
      'The snatch bot icon is not showing pop up circle'
    );
  });
  ...
  it('2. Verify snatch bot icon showing pop up message box', async () => {
    ...
    MochaUtils.verifyIsTrue(
      eleExists,
      'The snatch bot icon is not showing pop up message box'
    );
  });

  it('3. Verify if correct message is showing up in snatch bot pop up message box', async () => {
    const actualText = await DriverUtils.getText(
      driver,
      HOMEPAGE_LOCATOR.snatchBotPopUpBox
    );
